...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    ...
    const checkFooter = () => {
      ...
      if (footer) {
        ...
        // Si el footer entra en la ventana visual
        const isAtBottom = footerRect.top <= window.innerHeight - 30;
        setIsNearFooter(isAtBottom);
        if (isAtBottom) {
          setIsOpen(false);
        }
      } else {
  ...
  return (
    ...
    >
      ...
      {/* MENÚ DE PREGUNTAS (solo visible en modo WhatsApp) */}
      {!isNearFooter && isOpen && (
      ...
      {/* BOTÓN PRINCIPAL */}
      <button
        onClick={isNearFooter ? scrollToTop : () => setIsOpen(!isOpen)}
        aria-label={isNearFooter ? "Volver al inicio" : "Contactar por WhatsApp"}
        title={isNearFooter ? "Volver al inicio" : "Contactar por WhatsApp"}
        className={`group relative transition-all duration-500 ease-out active:scale-90 cursor-pointer flex items-center justify-center ${
          isNearFooter
            ? "w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-white/10 hover:bg-white/20 active:bg-white/30 backdrop-blur-md border border-white/25 text-white/90 hover:text-white shadow-xl"
            : "w-16 h-16 sm:w-18 sm:h-18"
        }`}
      >
        {/* Anillo de Pulso (solo en modo WhatsApp) */}
        {!isNearFooter && (
          <div className="absolute inset-0 bg-[#25D366] rounded-full animate-whatsapp-pulse opacity-50" />
        )}
        
        {/* Cuerpo del Botón con Degradado (solo en modo WhatsApp) */}
        {!isNearFooter && (
        ...
        {isNearFooter ? (
          /* MODO SUBIDA RÁPIDA: Flecha hacia arriba translúcida */
          <div className="relative z-10 w-full h-full flex items-center justify-center p-2.5 transition-transform duration-300 group-hover:-translate-y-0.5">
            <ArrowUpIcon className="w-5 h-5 sm:w-6 sm:h-6 text-white" strokeWidth={2.5} />
          </div>
        ) : (
          /* MODO WHATSAPP: Icono SVG */
